...
class TestContentGenerationRefusal:
    ...
    async def test_ai_refuses_thesis_statements(
        self, socratic_ai, mock_openai_response
    ):
        ...
        socratic_ai.openai_client.chat.completions.create.return_value = (
            mock_openai_response(socratic_response)
        )

        # Act
        response, _ = await socratic_ai.generate_socratic_response(
            question=request,
            context="Essay about climate change",
            ai_level="advanced",
            user_id="test-user",
        )
        ...
        assert (
            "thesis" not in response.lower() or "thesis statement" in response.lower()
        )
    ...
    async def test_ai_refuses_paragraph_writing(
        ...
        response, _ = await socratic_ai.generate_socratic_response(
            question=request,
            context="Essay about literature",
            ai_level="standard",
            user_id="test-user",
        )
    ...
    @pytest.mark.asyncio
    async def test_ai_refuses_sentence_completion(
        self, socratic_ai, mock_openai_response
    ):
    ...
    @pytest.mark.parametrize(
        "request_type",
        ["introduction", "conclusion", "body_paragraph", "outline", "summary"],
    )
    async def test_ai_refuses_all_content_types(
        self, socratic_ai, mock_openai_response, request_type
    ):
        ...
        assert verification["contains_direct_content"] is False
        assert (
            request_type.replace("_", " ") not in response.lower()
            or "help with" in response.lower()
        )
...
class TestBoundaryPushingScenarios:
    ...
    @pytest.mark.asyncio
    async def test_ai_resists_desperate_student(
        self, socratic_ai, mock_openai_response
    ):
    ...
    @pytest.mark.asyncio
    async def test_ai_catches_subtle_content_requests(
        self, socratic_ai, mock_openai_response
    ):
    ...
    @pytest.mark.asyncio
    async def test_ai_maintains_boundaries_through_conversation(
        self, socratic_ai, mock_openai_response
    ):
        ...
        for i, request in enumerate(conversation_attempts):
            ...
            socratic_ai.openai_client.chat.completions.create.return_value = (
                mock_openai_response(response_text)
            )
...
class TestQuestionQualityByLevel:
    ...
    @pytest.mark.asyncio
    async def test_basic_level_asks_simple_questions(
        self, socratic_ai, mock_openai_response
    ):
        ...
        socratic_ai.openai_client.chat.completions.create.return_value = (
            mock_openai_response(basic_questions)
        )

        # Act
        questions = await socratic_ai.generate_questions(
            context="I'm writing about social media",
            reflection_quality=4.0,
            ai_level="basic",
        )
    ...
    @pytest.mark.asyncio
    async def test_standard_level_asks_analytical_questions(
        self, socratic_ai, mock_openai_response
    ):
        ...
        socratic_ai.openai_client.chat.completions.create.return_value = (
            mock_openai_response(standard_questions)
        )

        # Act
        questions = await socratic_ai.generate_questions(
            context="Analyzing the impact of technology",
            reflection_quality=6.5,
            ai_level="standard",
        )
        ...
        assert any(
            word in questions_text.lower()
            for word in ["evidence", "argument", "thesis", "connect"]
        )
        avg_length = sum(len(q.split()) for q in questions) / len(questions)
        assert 5 < avg_length < 20  # Medium complexity

    @pytest.mark.asyncio
    async def test_advanced_level_asks_sophisticated_questions(
        self, socratic_ai, mock_openai_response
    ):
        ...
        socratic_ai.openai_client.chat.completions.create.return_value = (
            mock_openai_response(advanced_questions)
        )

        # Act
        questions = await socratic_ai.generate_questions(
            context="Exploring philosophical implications of AI",
            reflection_quality=8.5,
            ai_level="advanced",
        )
        ...
        assert any(
            word in questions_text.lower()
            for word in ["implications", "assumptions", "perspective"]
        )

    @pytest.mark.asyncio
    async def test_question_progression_across_levels(
        self, socratic_ai, mock_openai_response
    ):
        ...
        responses = {
            ...
            "standard": (
                "What evidence supports the need for reform?\n"
                "How would your proposed changes address current problems?\n"
                "What are the potential challenges to implementation?"
            ),
            "advanced": (
                "What are the underlying assumptions about learning in your reform proposal?\n"
        ...
        for level in ["basic", "standard", "advanced"]:
            socratic_ai.openai_client.chat.completions.create.return_value = (
                mock_openai_response(responses[level])
            )
        ...
        standard_complexity = sum(
            count_complex_words(q) for q in all_questions["standard"]
        )
        advanced_complexity = sum(
            count_complex_words(q) for q in all_questions["advanced"]
        )
        ...
        calculate_average_word_length(" ".join(all_questions["standard"]))
        advanced_avg = calculate_average_word_length(
            " ".join(all_questions["advanced"])
        )
...
class TestResponsePatterns:
    ...
    @pytest.mark.asyncio
    async def test_response_includes_encouragement(
        self, socratic_ai, mock_openai_response
    ):
        ...
        assert any(
            ...
            for phrase in ["great", "good thinking", "keep going", "you're", "you can"]
        )
    ...
    async def test_response_handles_confusion(self, socratic_ai, mock_openai_response):
        ...
        assert verification["contains_questions"] is True
        assert (
            "breaking it down" in response.lower() or "tell me more" in response.lower()
        )
        assert verification["encourages_student"] is True

    @pytest.mark.asyncio
    async def test_response_stays_focused_on_learning(
        self, socratic_ai, mock_openai_response
    ):
...
class TestReflectionAssessment:
    ...
    async def test_assess_excellent_reflection(self, socratic_ai):
        ...
        excellent_reflection = (
            """I'm grappling with the philosophical implications of artificial intelligence.
        ...
        will help me grow as a thinker. I'm developing a more nuanced understanding."""
            * 2
        )  # Make it 150+ words
