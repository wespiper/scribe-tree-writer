...
async def create_test_user(
    client: AsyncClient, email: str = "test@example.com", password: str = "testpass123", full_name: str = "Test User"
) -> dict[str, Any]:
    ...
    return {"Authorization": f"Bearer {token}", "user_data": response.json(), "token": token}
...
async def create_test_document(
    ...
    response = await client.post("/api/documents/", json=document_data, headers=auth_headers)
    assert response.status_code == 200
...
async def create_test_document_in_db(
    db_session: AsyncSession, user_id: str, title: str = "Test Document", content: str = "This is test content."
) -> Document:
    """Create a document directly in the database"""
    document = Document(user_id=user_id, title=title, content=content, word_count=len(content.split()))
...
async def submit_reflection(
    client: AsyncClient, auth_headers: dict[str, str], document_id: str, reflection_content: Optional[str] = None
) -> dict[str, Any]:
    ...
    response = await client.post("/api/ai/reflect", json=reflection_data, headers=auth_headers)
...
async def get_ai_response(
    ...
    response = await client.post("/api/ai/chat", json=request_data, headers=auth_headers)
...
def assert_is_question(text: str) -> None:
    ...
    question_indicators = ["?", "what", "how", "why", "when", "where", "which", "could", "would", "should"]
...
def assert_no_direct_answers(text: str) -> None:
    ...
    for phrase in forbidden_phrases:
        assert phrase not in text_lower, f"AI response contains forbidden phrase '{phrase}': {text}"
