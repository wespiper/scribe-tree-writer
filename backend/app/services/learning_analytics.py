...
class LearningAnalyticsService:
    ...
    async def track_reflection(self, user_id: str, document_id: str, quality_score: float, ai_level: str):
    ...
    async def track_ai_interaction(self, user_id: str, document_id: str, interaction_type: str, response_time_ms: int):
    ...
    async def calculate_learning_metrics(self, user_id: str, db: AsyncSession) -> dict:
        ...
        result = await db.execute(
            select(Reflection.quality_score, Reflection.created_at)
        ...
        if len(reflections) >= 2:
            ...
            recent_avg = sum(r.quality_score for r in reflections[len(reflections) // 2 :]) / (
                len(reflections) - len(reflections) // 2
            )
        ...
        result = await db.execute(
            select(func.count(Document.id).label("total_documents")).where(Document.user_id == user_id)
        )
        total_documents = result.scalar() or 0

        result = await db.execute(
            select(func.count(func.distinct(AIInteraction.document_id)).label("documents_with_ai")).where(
        ...
        return {
            ...
            "average_reflection_score": sum(r.quality_score for r in reflections) / len(reflections)
            if reflections
            else 0,
    ...
    async def get_document_analytics(self, document_id: str, db: AsyncSession) -> dict:
        ...
        result = await db.execute(select(func.count(AIInteraction.id)).where(AIInteraction.document_id == document_id))
        interaction_count = result.scalar() or 0

        # Get reflection history
        result = await db.execute(
            select(Reflection).where(Reflection.document_id == document_id).order_by(Reflection.created_at.desc())
        )
